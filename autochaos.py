...
class AutoClickerGtkApp(Gtk.Application):
    def __init__(self, **kwargs):
        ...
        self.track_distance = 10  # Default tracking distance in pixels
    ...
    def load_settings(self):
        ...
        default_settings = {
            ...
            'color_delay_ms': 1500,
            'selected_colors': [],
            'track_distance': 10  # Default 10 pixels
        }
        try:
            if os.path.exists('autoclick.json'):
                with open('autoclick.json', 'r') as f:
                    ...
                    self.track_distance = settings.get('track_distance', default_settings['track_distance'])
                    # Update UI
                    if hasattr(self, 'track_distance_entry'):
                        self.track_distance_entry.set_text(str(int(self.track_distance)))
            else:
    ...
    def apply_default_settings(self, default_settings):
        ...
        self.track_distance = default_settings['track_distance']
        self.save_settings()
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            'track_distance': int(self.track_distance)
        }
        try:
    ...
    def build_ui(self, app):
        ...
        # Track Distance Input
        track_distance_label = Gtk.Label(label="Tıklama Alanı İzleme Mesafesi (piksel):")
        self.track_distance_entry = Gtk.Entry()
        self.track_distance_entry.set_text(str(int(self.track_distance)))
        self.track_distance_entry.set_width_chars(10)
        self.track_distance_entry.set_input_purpose(Gtk.InputPurpose.NUMBER)
        self.track_distance_entry.connect("changed", self.on_track_distance_changed)

        track_distance_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        track_distance_box.pack_start(track_distance_label, False, False, 0)
        track_distance_box.pack_start(self.track_distance_entry, True, True, 0)
        main_box.pack_start(track_distance_box, False, False, 0)
    ...
    def on_track_distance_changed(self, entry):
        try:
    ...
    def on_start_button_clicked(self, button):
        ...
        if self.clicking:
            ...
            self.click_position = None  # Reset click position to force new coordinate
            GLib.idle_add(self.start_waiting)
    ...
    def stop_waiting(self):
        ...
        if self.mouse_listener:
            self.mouse_listener.stop()
            self.mouse_listener = None
        self.start_button.set_label("Çoklu Tıklayıcıyı Başlat")
        self.start_button.set_sensitive(True)
        self.color_picker_button.set_sensitive(self.color_detection_enabled)
        self.edit_colors_button.set_sensitive(self.color_detection_enabled)
        self.status_label.set_label("Durum: Kaos Tıklayıcı Kapalı\n\nSen bulgura kaşık sallarken,\nsenin yerine evlendiği potansiyel\neşine hiç bir işe yaramadan maaşıyla\nbakmaya devam ediyor Troller")
        self.status_label.get_style_context().remove_class("success-label")
        self.status_label.get_style_context().remove_class("warning-label")
        self.status_label.get_style_context().add_class("error-label")
        self.progress_bar.set_fraction(0.0)
    ...
    def auto_click(self):
        ...
        def find_new_position(current_x, current_y, target_colors, search_range):
            for dx in range(-search_range, search_range + 1, 2):
                for dy in range(-search_range, search_range + 1, 2):
                    try:
                        pixel = pyautogui.pixel(current_x + dx, current_y + dy)
        ...
        while self.clicking:
            ...
            if self.click_position:
                ...
                if first_click or click_counter % 10 == 0 or is_color_match:
                    self.click_position = find_new_position(
                        self.click_position[0], self.click_position[1], self.selected_colors, self.track_distance
                    )
                    first_click = False
                ...
                try:
                    ...
                    click_duration = click_time - last_click_time
                ...
                if is_color_match or click_duration <= self.click_interval * 1.1:
                    update_start = time.time()
                    GLib.idle_add(self.update_progress_gui_update)
                    last_update_duration = time.time() - update_start
                    if is_color_match and last_update_duration < self.color_delay:
                        time.sleep(self.color_delay - last_update_duration)
    ...
    def update_progress_gui_update(self):
        if self.total_clicks > 0:
            ...
            self.progress_bar.set_fraction((current_fraction + 0.1) % 1.0)
