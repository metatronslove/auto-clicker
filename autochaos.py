...
class AutoClickerGtkApp(Gtk.Application):
        ...
        try:
            if os.path.exists('autoclick.json'):
                with open('autoclick.json', 'r') as f:
                    ...
                    # Ensure selected_colors is a list of RGB tuples
    ...
    def on_edit_colors_clicked(self, button):
        ...
        # Swatch column
        swatch_renderer = Gtk.CellRendererText()
        swatch_column = Gtk.TreeViewColumn("Renk", swatch_renderer)
        swatch_column.set_cell_data_func(swatch_renderer, self.render_color_swatch)
    ...
    def render_color_swatch(self, column, cell, model, iter, data):
        """Render a color swatch in the TreeView."""
        r = model.get_value(iter, 1)
        g = model.get_value(iter, 2)
        b = model.get_value(iter, 3)
        cell.set_property('text', '')
        cell.set_property('cell-background-rgb', Gdk.RGBA(r / 255.0, g / 255.0, b / 255.0, 1.0))
    ...
    def update_colors_label(self):
        """Update the colors display with swatches and RGB values."""
        ...
        if self.selected_colors:
            ...
            for r, g, b in self.selected_colors:
                ...
                # Add RGB label
                label = Gtk.Label(label=f"RGB({r},{g},{b})")
    ...
    def auto_click(self):
        while self.clicking:
            ...
            if self.click_position:
                # Default to regular click interval
                current_interval = self.click_interval
                # Check color if color detection is enabled and there are selected colors
                if self.color_detection_enabled and self.selected_colors:
                    try:
                        ...
                        print(f"Checked color at ({self.click_position[0]}, {self.click_position[1]}): RGB({pixel[0]}, {pixel[1]}, {pixel[2]})")
                        if pixel in self.selected_colors:
                            current_interval = self.color_delay
                            print(f"Color match, using color_delay: {self.color_delay}s")
                        else:
                            print(f"No color match, using click_interval: {self.click_interval}s")
                    except Exception as e:
                        print(f"Error getting pixel color: {e}")
                        # Fallback to click_interval on error
                        current_interval = self.click_interval
